...
export default class FirebaseClass
{
    ...
    async uploadRecording(email: string, file: File)
    {
        return new Promise<string>((resolve, reject) =>
        {
            const fileRef = ref(
                this.storage,
                `recordings/${email.split("@")[0]}/${file.name.split(".")[0]}-${file.name.split(".")[1]
                }`
            );
    ...
    async initializeCall(caller_id: string, receiver_id: string, receiver_number: string)
    {
    ...
    async getCallDetails(call_id: string)
    {
        try
        {
            ...
            if (details != undefined)
                return details as TCallDetails;
            else
